...
def create_cat(db_collection):
    ...
    features = input("Enter the cat's feature (use ';' to split multiple entries): ")
...
def add_feature_to_cat(db_collection):
    ...
    result = db_collection.update_one({"name": name}, {"$addToSet": {"features": new_feature}})
    if result.matched_count > 0:
...
def main():
    """Main function of all crud operations that provides CLI interface for the user"""
